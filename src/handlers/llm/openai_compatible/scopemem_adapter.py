...
def _clip_for_log(text: Any, limit: int = 180) -> str:
...
class OpenAvatarScopeMemory:
    _cleared_store_keys: set[str] = set()
    ...
    def __init__(
        ...
        *,
        ...
        if clear_on_start:
            self._clear_store_once(store_path_obj, qdrant_path)
    ...
    @classmethod
    def _clear_store_once(cls, store_path: Path, qdrant_path: Path) -> None:
        key = str(store_path.resolve())
        if key in cls._cleared_store_keys:
            return
        cls._cleared_store_keys.add(key)
        try:
            store_path.unlink(missing_ok=True)
            if qdrant_path.exists():
                shutil.rmtree(qdrant_path)
            logger.info(
                "ScopeMem cleared previous startup memory: "
                f"jsonl={store_path} index={qdrant_path}"
            )
        except Exception as exc:
            logger.warning(f"ScopeMem failed to clear previous startup memory: {exc}")

    def build_system_prompt(self, system_prompt: Dict[str, str], query: str) -> Dict[str, str]:
        memories = self.search(query)
        if not memories:
            logger.info(f"ScopeMem used memories: count=0 query={_clip_for_log(query)!r}")
        ...
        for index, item, text in used_items:
            ...
            details.append(f"#{index}{score_text} text={_clip_for_log(text)!r}")
        logger.info(
            "ScopeMem used memories: "
            f"count={len(used_items)} query={_clip_for_log(query)!r} "
    ...
    def remember_turn(self, user_text: str, assistant_text: str) -> None:
        ...
        turn_index = (len(self.pending_messages) // 2) + 1
        messages = [
            {
                "role": "user",
                ...
                "dia_id": f"turn_{turn_index}_user",
            ...
            },
            {
                ...
                "timestamp": timestamp,
                "session_key": "openavatarchat",
                "dia_id": f"turn_{turn_index}_assistant",
        ...
        logger.info(
            "ScopeMem buffered turn: "
            f"pending_messages={len(self.pending_messages)}/{self.extract_batch_size}"
        )
        while len(self.pending_messages) >= self.extract_batch_size:
            batch = self.pending_messages[: self.extract_batch_size]
            del self.pending_messages[: self.extract_batch_size]
            batch_timestamp = self.pending_since or timestamp
            self.pending_since = self.pending_messages[0].get("timestamp", "") if self.pending_messages else ""
            self._submit_messages(batch, batch_timestamp, reason="batch_full")
    ...
    def flush(self, reason: str = "flush") -> None:
        ...
        batch_timestamp = self.pending_since or datetime.now(timezone.utc).isoformat()
        self.pending_since = ""
        self._submit_messages(batch, batch_timestamp, reason=reason)

    def _submit_messages(self, messages: List[Dict[str, Any]], timestamp: str, reason: str) -> None:
        if not messages:
            return
        logger.info(
