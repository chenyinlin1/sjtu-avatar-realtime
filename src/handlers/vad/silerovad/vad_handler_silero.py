...
def _prepare_vad_model_clip(clip: np.ndarray) -> np.ndarray:
    clip = clip.astype(np.float32, copy=False)
    if clip.size == 0:
        return clip
    max_abs = float(np.max(np.abs(clip)))
    if max_abs > 1.5:
        return np.clip(clip / 32768.0, -1.0, 1.0).astype(np.float32, copy=False)
    return clip


def _get_vad_model_context_size(sr: int) -> int:
    return 64 if sr == 16000 else 32


def _build_vad_model_input(
        context: "HumanAudioVADContext",
        prepared_clip: np.ndarray,
        sr: int) -> np.ndarray:
    context_size = _get_vad_model_context_size(sr)
    if context.model_context is None or context.model_context.shape[0] != context_size:
        context.model_context = np.zeros(context_size, dtype=np.float32)

    model_clip = np.concatenate((context.model_context, prepared_clip), axis=0)
    context.model_context = model_clip[-context_size:].copy()
    return np.expand_dims(model_clip, axis=0)


def _get_vad_clip_stats(clip: np.ndarray, model_clip: np.ndarray) -> Dict[str, float]:
    input_peak = float(np.max(np.abs(clip))) if clip.size else 0.0
    input_rms = float(np.sqrt(np.mean(clip.astype(np.float32) ** 2))) if clip.size else 0.0
    model_peak = float(np.max(np.abs(model_clip))) if model_clip.size else 0.0
    return {
...
def _apply_energy_speech_fallback(
        speech_prob: float,
        db: float,
        config: "SileroVADConfigModel") -> Tuple[float, bool]:
    if config.energy_speech_threshold is None:
        return speech_prob, False
    if speech_prob > config.speaking_threshold:
        return speech_prob, False
    if db < config.energy_speech_threshold:
        return speech_prob, False
    return config.speaking_threshold + 1e-3, True
...
class SileroVADConfigModel(HandlerBaseConfigModel, BaseModel):
    ...
    energy_speech_threshold: Optional[float] = Field(
        default=None,
        description="通用能量兜底阈值（dB）；高于此值时可在模型概率偏低的情况下触发语音",
    )
...
class HumanAudioVADContext(HandlerContext):
    def __init__(self, session_id: str):
        ...
        self.model_context: Optional[np.ndarray] = None
    ...
    def reset_model(self):
        ...
        self.model_context = np.zeros(_get_vad_model_context_size(16000), dtype=np.float32)
    ...
    def _update_status_on_end(self, _clip: np.ndarray, _timestamp: Optional[int] = None):
        if self.speech_length > 0:
            ...
            self.speaking_status = SpeakingStatus.PRE_START
            return self._update_status_on_pre_start(_clip, _timestamp)
        return None, {}
...
class HandlerAudioVAD(HandlerBase, ABC):
    ...
    def _inference(self, context: HumanAudioVADContext, clip: np.ndarray, sr: int=16000):
        ...
        if clip.ndim != 1:
            ...
            return 0
        prepared_clip = _prepare_vad_model_clip(clip)
        context._last_vad_clip_stats = _get_vad_clip_stats(clip, prepared_clip)
        model_clip = _build_vad_model_input(context, prepared_clip, sr)
        inputs = {
            "input": model_clip,
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        for clip in slice_data(context.slice_context, audio):
            ...
            speech_prob = self._inference(context, clip, sample_rate)
            raw_speech_prob = speech_prob
            ...
            if context.speaking_status in (
                    SpeakingStatus.END,
                    SpeakingStatus.PRE_START,
                    SpeakingStatus.START,
                    SpeakingStatus.POST_END):
                speech_prob, energy_fallback_used = _apply_energy_speech_fallback(
                    speech_prob=speech_prob,
                    db=db,
                    config=context.config,
                )
            now = time.time()
            last_vad_diag = getattr(context, "_last_vad_diag_log", 0.0)
            if now - last_vad_diag >= 1.0:
                context._last_vad_diag_log = now
                logger.info(
                    f"VAD diag: session={context.session_id}, status={context.speaking_status.name}, "
                    f"db={db:.1f}, raw_prob={raw_speech_prob:.3f}, prob={speech_prob:.3f}, "
                    f"threshold={context.config.speaking_threshold}, volume_threshold={context.config.volume_threshold}, "
                    f"energy_fallback={energy_fallback_used}, "
                    f"input_peak={getattr(context, '_last_vad_clip_stats', {}).get('input_peak', 0.0):.4f}, "
                    f"input_rms={getattr(context, '_last_vad_clip_stats', {}).get('input_rms', 0.0):.4f}, "
                    f"model_peak={getattr(context, '_last_vad_clip_stats', {}).get('model_peak', 0.0):.1f}"
                )
