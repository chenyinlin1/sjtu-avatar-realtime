...
class DuplexVADHandler(HandlerAudioVAD):
    ...
    def _handle_audio_input(self, context: DuplexVADContext, inputs: ChatData, 
        ...
        for clip in slice_data(context.slice_context, audio):
            ...
            now = time.time()
            last_vad_diag = getattr(context, "_last_duplex_vad_diag_log", 0.0)
            if now - last_vad_diag >= 1.0:
                context._last_duplex_vad_diag_log = now
                logger.info(
                    f"Duplex VAD diag: session={context.session_id}, status={context.speaking_status.name}, "
                    ...
                    f"model_peak={getattr(context, '_last_vad_clip_stats', {}).get('model_peak', 0.0):.4f}"
                )
