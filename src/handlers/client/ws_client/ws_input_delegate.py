...
class WsInputSessionDelegate(ClientSessionDelegate):
    ...
    async def _handle_audio_data(self, websocket: WebSocket, msg: SendHumanAudio, audio_bytes: bytes):
        ...
        try:
            ...
            audio_peak = int(np.max(np.abs(audio_array))) if audio_array.size else 0
            audio_rms = float(np.sqrt(np.mean(audio_array.astype(np.float32) ** 2))) if audio_array.size else 0.0
            now = time.time()
            last_log = getattr(self, "_last_audio_diag_log", 0.0)
            if now - last_log >= 1.0:
                self._last_audio_diag_log = now
                logger.info(
                    f"WS mic audio received: session={self.session_id}, samples={audio_array.size}, "
                    f"format={audio_format}, peak={audio_peak}, rms={audio_rms:.1f}"
                )
