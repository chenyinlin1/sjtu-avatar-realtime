...
def _mask_rtc_config_for_logging(config: Dict) -> Dict:
    masked = dict(config)
    if "credential" in masked:
        masked["credential"] = "***masked***"
    if "twilio_auth_token" in masked:
        masked["twilio_auth_token"] = "***masked***"
    return masked
...
class RTCProvider(metaclass=SingletonMeta):
    ...
    def prepare_rtc_configuration(self, config: Union[ServiceConfigData, BaseModel, Dict]):
        ...
        if rtc_config is not None:
            logger.info(f"Parsing RTC config: {_mask_rtc_config_for_logging(rtc_config)}")
