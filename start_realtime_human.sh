...
WEB_SEARCH_ALWAYS="${OPENAVATAR_WEB_SEARCH_ALWAYS:-false}"  # 默认情况下，Bocha 模式下只在关键字触发时搜索，而不是每次用户请求都搜索。
...
# Sample AV-sync diagnostics to keep per-frame logging overhead bounded.
# Override with AV_SYNC_DIAG=0 to disable or AV_SYNC_DIAG_EVERY=N to change sampling.
AV_SYNC_DIAG="${AV_SYNC_DIAG:-1}"
AV_SYNC_DIAG_EVERY="${AV_SYNC_DIAG_EVERY:-25}"
AV_SYNC_RTP_VIDEO_LEAD_LIMIT_MS="${AV_SYNC_RTP_VIDEO_LEAD_LIMIT_MS:-40}"
...
SEARCH_CALL_LOG="${OPENAVATAR_SEARCH_CALL_LOG:-$LOG_DIR/search_logs/search_calls_${PORT}.jsonl}"
...
export OPENAVATAR_SEARCH_CALL_LOG="$SEARCH_CALL_LOG"
export OPENAVATAR_ENABLE_EMOTIONAL_SUPPORT_SKILLS="$EMOTIONAL_SUPPORT_SKILLS"
export AV_SYNC_DIAG
export AV_SYNC_DIAG_EVERY
export AV_SYNC_RTP_VIDEO_LEAD_LIMIT_MS
...
echo "SEARCH_CALL_LOG: $SEARCH_CALL_LOG"
echo "EMOTIONAL_SUPPORT_SKILLS: $EMOTIONAL_SUPPORT_SKILLS"
echo "AV_SYNC_DIAG: $AV_SYNC_DIAG"
echo "AV_SYNC_DIAG_EVERY: $AV_SYNC_DIAG_EVERY"
echo "AV_SYNC_RTP_VIDEO_LEAD_LIMIT_MS: $AV_SYNC_RTP_VIDEO_LEAD_LIMIT_MS"
echo
