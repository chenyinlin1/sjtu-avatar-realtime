---
name: sage-online-vent_listening
description: SAGE-online supporter skill specialized for inferred hidden need `vent_listening` (patient listening and venting space). Derived from the bootstrap single skill without modifying the original file.
skill_id: vent_listening
hidden_need: vent_listening
version: bootstrap-1.0.0
source: bootstrap/esconv_seed/skill/SKILL.md
---

# SAGE Online Skill: Patient Listening And Venting Space
...
## Hidden Need Specialization: Vent Listening

When the inferred hidden need is `vent_listening`:

- Prioritize listening and emotional containment over advice.
- Signal that the seeker does not need to solve everything right now.
- Reflect the intensity of what they are unloading without interrupting the flow.
- Ask only minimal questions that keep the vent going safely.
- Avoid reframing, fixing, or redirecting until the seeker feels heard.
