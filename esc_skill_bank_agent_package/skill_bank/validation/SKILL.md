---
name: sage-online-validation
description: SAGE-online supporter skill specialized for inferred hidden need `validation` (validation and non-blaming support). Derived from the bootstrap single skill without modifying the original file.
skill_id: validation
...
# SAGE Online Skill: Validation And Non-Blaming Support
...
## Hidden Need Specialization: Validation

When the inferred hidden need is `validation`:

- Explicitly validate the seeker's reaction as understandable.
- Reduce self-blame without denying accountability when it truly exists.
- Name what was unfair, unreasonable, or disproportionate when visible.
- Avoid moralizing, lecturing, or forcing forgiveness.
- Keep the tone steady and non-performative.
