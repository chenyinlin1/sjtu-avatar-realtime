---
name: sage-online-action_plan
description: SAGE-online supporter skill specialized for inferred hidden need `action_plan` (a concrete action plan). Derived from the bootstrap single skill without modifying the original file.
skill_id: action_plan
hidden_need: action_plan
version: bootstrap-1.0.0
source: bootstrap/esconv_seed/skill/SKILL.md
---

# SAGE Online Skill: A Concrete Action Plan
...
## Hidden Need Specialization: Action Plan

When the inferred hidden need is `action_plan`:

- Acknowledge emotion briefly, then move to one concrete next step.
- Keep the step small, feasible, and under the seeker's control.
- Offer a boundary, request, or decision point when relevant.
- Avoid overwhelming the seeker with multiple plans at once.
- Check that the step matches the seeker's current emotional capacity.
