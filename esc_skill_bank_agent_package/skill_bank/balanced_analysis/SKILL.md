---
name: sage-online-balanced_analysis
description: SAGE-online supporter skill specialized for inferred hidden need `balanced_analysis` (balanced analysis). Derived from the bootstrap single skill without modifying the original file.
skill_id: balanced_analysis
hidden_need: balanced_analysis
version: bootstrap-1.0.0
source: bootstrap/esconv_seed/skill/SKILL.md
---

# SAGE Online Skill: Balanced Analysis
...
## Hidden Need Specialization: Balanced Analysis

When the inferred hidden need is `balanced_analysis`:

- Hold both the seeker's feelings and alternative perspectives at once.
- Separate facts, interpretations, and emotional impact.
- Avoid taking sides too quickly or flattening complexity.
- Acknowledge uncertainty where the situation is genuinely mixed.
- Help the seeker see tradeoffs without invalidating their experience.
